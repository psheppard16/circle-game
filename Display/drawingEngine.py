...
class DrawingEngine(Screen):
    ...
    def render(self, player, circleList, starList):
    ...
    def showPlayer(self, player):
        self.showCircle(player.radius * self.window.gameEngine.scale, (self.getScreenX(player.x), self.getScreenY(player.y)), (0, 0, 0))

    def showGUI(self):
        self.showText("Stars collected:" + str(self.window.gameEngine.player.starsCollected), (self.window.width / 4, 50), (255, 255, 0), fontSize=50, bold=True, anchorCenter=True, shadowWidth=4)
        self.showText("Time Left:" + str(int(self.window.gameEngine.timeLeft)), (self.window.width / 4 * 3, 50), (255, 0, 255), fontSize=50, bold=True, anchorCenter=True, shadowWidth=4)

    def showCircles(self, circleList):
        for circle in circleList:
            self.showCircle(circle.radius * self.window.gameEngine.scale, (self.getScreenX(circle.x), self.getScreenY(circle.y)), circle.color)

    def showStars(self, starList):
        for s in starList:
            self.showPolygon(s.pointList, (255, 255, 255), (self.getScreenX(s.x), self.getScreenY(s.y)))
    ...
    def getScreenX(self, x):
        return x * self.window.gameEngine.scale

    def getScreenY(self, y):
        return self.window.height - y * self.window.gameEngine.scale