...
class Options(Screen):
    def __init__(self, window):
        ...
        self.difficultyF = StringVar()
        if self.window.save.difficulty == "easy":
            self.difficultyF.set("Difficulty: Easy")
        elif self.window.save.difficulty == "medium":
            self.difficultyF.set("Difficulty: Medium")
        elif self.window.save.difficulty == "hard":
            self.difficultyF.set("Difficulty: Hard")
        self.difficultyB = Button(self.window.root, textvariable=self.difficultyF, command=self.showOutline, bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold", padx=10, pady=10)
        self.difficultyB.pack(in_=self.f, pady=15)
    ...
    def showOutline(self):
        if self.window.save.difficulty == "easy":
            self.window.save.difficulty = "medium"
            self.difficultyF.set("Difficulty: Medium")
        elif self.window.save.difficulty == "medium":
            self.window.save.difficulty = "hard"
            self.difficultyF.set("Difficulty: Hard")
        elif self.window.save.difficulty == "hard":
            self.window.save.difficulty = "easy"
            self.difficultyF.set("Difficulty: Easy")
